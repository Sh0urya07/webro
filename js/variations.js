...
function applyVariation() {
  ...
  if (!reduced) {
    document.querySelectorAll('.bg-fx .blob').forEach((b) => {
      // transform-only movement: repositioning with top/left after first
      // paint counts as a layout shift (CLS); translate does not.
      const tx = Math.round(rand(-25, 25));
      const ty = Math.round(rand(-25, 25));
      b.style.transform = `translate(${tx}vw, ${ty}vh) scale(${rand(0.8, 1.3).toFixed(2)})`;
